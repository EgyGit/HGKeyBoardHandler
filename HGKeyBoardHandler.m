...
#import "HGKeyBoardHandler.h"

#define HGKEYBOARD_LOG_ON

#ifdef  HGKEYBOARD_LOG_ON
#define KEYBOARD_LOG(fmt, ...) { NSLog(@"- HGKeyBoardHandlerLog - "fmt, ##__VA_ARGS__); }
#else
#define KEYBOARD_LOG(fmt, ...) {}
#endif


@interface HGKeyBoardHandler () <UIGestureRecognizerDelegate>
...
@implementation HGKeyBoardHandler
{
...
{
    KEYBOARD_LOG(@"Will show Keyboard!");
...
{
    KEYBOARD_LOG(@"Will hidden Keyboard!");
...
- (void)resignFirstResponder
{
    if (self.textFieldView && [self.textFieldView isFirstResponder])
    {
        ...
        if (!temp) {
            KEYBOARD_LOG(@"reject resign!");
        } else
        {
            KEYBOARD_LOG(@"resign First Responder!");
        }
...
{
//    KEYBOARD_LOG(@"-----Simultaneously------<%@, %@>", gestureRecognizer, otherGestureRecognizer);
...
- (void)dealloc
{
    KEYBOARD_LOG(@"keyboardHandler dealloc!");
    if (_window) [_window removeGestureRecognizer:self.tapGesture];
